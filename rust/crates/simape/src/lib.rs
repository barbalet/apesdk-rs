...
pub fn save_open_continuity_fixture() -> Result<String, &'static str> {
    ...
    output.push('\n');
    output.push_str(&save_open_sidecar_line("before_save", &state));
    output.push('\n');
    output.push_str(&save_open_trace_line("after_open", &loaded));
    output.push('\n');
    output.push_str(&save_open_sidecar_line("after_open", &loaded));
    output.push('\n');
    ...
    output.push('\n');
    output.push_str(&save_open_sidecar_line("continued_minute", &continued));
    output.push('\n');
    output.push_str(&save_open_trace_line("post_open_minute", &loaded));
    output.push('\n');
    output.push_str(&save_open_sidecar_line("post_open_minute", &loaded));
    output.push('\n');
    ...
    output.push('\n');
    output.push_str(&save_open_sidecar_line("continued_day", &continued));
    output.push('\n');
    output.push_str(&save_open_trace_line("post_open_day", &loaded));
    output.push('\n');
    output.push_str(&save_open_sidecar_line("post_open_day", &loaded));
    output.push('\n');
    Ok(output)
}
...
fn save_open_sidecar_line(label: &str, state: &SimState) -> String {
    selected_minute_trace_line(label, n_uint::from(state.land().time()), state).replacen(
        "SELECTED-MINUTE label=",
        "SAVE-OPEN-SIDECAR snapshot=",
        1,
    )
}
...
pub fn selected_minute_trace_line(label: &str, minute: n_uint, state: &SimState) -> String {
    ...
    let drives = selected.map(BeingSummary::drives).unwrap_or([0; 4]);
    let random_seed = selected.map(BeingSummary::random_seed).unwrap_or([0; 2]);
    let goal = selected.map(BeingSummary::goal).unwrap_or([0; 4]);
    let social_coord = selected.map(BeingSummary::social_coord).unwrap_or([0; 4]);
    let brain = selected
        .map(BeingSummary::braincode_register)
        .unwrap_or([0; 3]);
    let brain_state = selected.map(BeingSummary::brain_state).unwrap_or([0; 6]);
    let attention = selected.map(BeingSummary::attention).unwrap_or([0; 6]);
    let probe = selected
    ...
    let (
        ...
        let terrain = land.terrain_sample(location);
        let food = land.food_classification_at(location);
        (
            terrain.height,
            terrain.slope.x,
            terrain.slope.y,
            usize::from(terrain.water),
            usize::from(terrain.intertidal),
            food.food_type,
            food.max_energy,
        )
    } else {
    ...
    format!(
        "SELECTED-MINUTE label={label} minute={minute} date={} time={} population={} \
         selected={} name={}:{} energy={} loc={}:{} facing={} speed={}:{} honor={} \
         height={} mass={} posture={} parasites={} crowding={} awake={} state={} \
         drives={}:{}:{}:{} seed={}:{} goal={}:{}:{}:{} socialcoord={}:{}:{}:{} \
         brain={}:{}:{} brainstate={}:{}:{}:{}:{}:{} attention={}:{}:{}:{}:{}:{} \
         probe0={}:{}:{}:{}:{}:{} script={} shout={}:{}:{}:{}:{}:{} \
         social0={}:{}:{}:{}:{}:{} episodic0={}:{}:{}:{} territory0={}:{} \
         conception={} family={}:{}:{}:{}:{}:{} terrain={}:{}:{}:{}:{} food={}:{} \
         immune={}:{}:{}:{}:{}:{} inventory={}:{} preference={}",
        land.date(),
        land.time(),
        state.population(),
        ...
        selected.map(BeingSummary::parasites).unwrap_or(0),
        selected.map(BeingSummary::crowding).unwrap_or(0),
        ...
        probe.offset,
        probe.state,
        selected.map(BeingSummary::script_overrides).unwrap_or(0),
        shout[0],
        shout[1],
        shout[2],
        shout[3],
...
mod tests {
    ...
    fn fixture_manifest_parses_and_points_to_existing_files() {
        let manifest = include_str!("../../../../both/golden/fixture_manifest.txt");
        let entries = parse_fixture_manifest(manifest).unwrap();
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../both");
    ...
    fn help_transcript_matches_c_golden() {
        ...
        let actual = console.run_script(
            include_str!("../../../../both/golden/cli/sessions/help.commands"),
            true,
        );
        let expected = normalize(include_str!(
            "../../../../both/golden/cli/transcripts/help.txt"
        ));
    ...
    fn help_errors_transcript_matches_c_golden() {
        let mut console = Console::default();
        let actual = console.run_script(
            include_str!("../../../../both/golden/cli/sessions/help_errors.commands"),
            true,
        );
        let expected = normalize(include_str!(
            "../../../../both/golden/cli/transcripts/help_errors.txt"
        ));
        assert_eq!(actual, expected);
    }
    ...
    fn runtime_parity_transcript_matches_rust_golden() {
        ...
        let actual = normalize_cli_transcript(&console.run_script(
            include_str!("../../../../both/golden/cli/sessions/runtime_parity.commands"),
            true,
        ));
        let expected = normalize_cli_transcript(include_str!(
            "../../../../both/golden/cli/transcripts/runtime_parity.txt"
        ));
        assert_eq!(actual, expected);
    }
