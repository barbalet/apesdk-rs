...
for (( index = 1; index <= RUNS; index++ )); do
    ...
    run_pipe "$ROOT/target/debug/simape" "$COMMAND_DIR/closed_stdin.commands" "$RAW_DIR/rust/closed_stdin_$index.txt"

    "$ROOT/scripts/canonicalize_closed_stdin_transcript.sh" "$RAW_DIR/native/closed_stdin_$index.txt" > "$CANONICAL_DIR/native/closed_stdin_$index.txt"
    "$ROOT/scripts/canonicalize_closed_stdin_transcript.sh" "$RAW_DIR/rust/closed_stdin_$index.txt" > "$CANONICAL_DIR/rust/closed_stdin_$index.txt"
