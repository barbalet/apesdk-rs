...
if [ "${APESDK_RUN_CORPUS_RAW_DIFFS:-0}" = "1" ]; then
    APESDK_RUN_PENDING_CORPUS_DIFFS=1 "$ROOT/scripts/run_pending_corpus_inventory.sh" "$PENDING_DIR" >/dev/null
else
    "$ROOT/scripts/run_pending_corpus_inventory.sh" "$PENDING_DIR" >/dev/null
fi
