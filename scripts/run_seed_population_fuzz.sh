...
run_pipe() {
    ...
    mkdir -p "$run_dir"
    ( cd "$run_dir" && "$ROOT/target/debug/simape" < "$commands" > "$output" ) || true
}
...
for commands in "$COMMAND_DIR"/*.commands; do
    ...
    "$ROOT/scripts/transport_normalize_transcript.sh" "$RAW_DIR/first/$name.txt" > "$TRANSPORT_DIR/first/$name.txt"
    "$ROOT/scripts/transport_normalize_transcript.sh" "$RAW_DIR/second/$name.txt" > "$TRANSPORT_DIR/second/$name.txt"
