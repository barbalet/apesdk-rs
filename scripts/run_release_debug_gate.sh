...
cargo build --manifest-path "$ROOT/Cargo.toml" --release -p simape

NATIVE_SIMAPE="$OUT_DIR/no-native-simape" \
RUST_SIMAPE="$ROOT/target/debug/simape" \
"$ROOT/scripts/run_cli_transcripts.sh" "$DEBUG_DIR"

NATIVE_SIMAPE="$OUT_DIR/no-native-simape" \
RUST_SIMAPE="$ROOT/target/release/simape" \
"$ROOT/scripts/run_cli_transcripts.sh" "$RELEASE_DIR"
