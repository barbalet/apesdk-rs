...
mkdir -p "$OUT_DIR"
APESDK_FULL_DATE="$FULL_DATE" "$ROOT/scripts/run_platform_absolute_parity_gate.sh" "$OUT_DIR/platform"
APESDK_FULL_DATE="$FULL_DATE" "$ROOT/scripts/run_absolute_parity_ci.sh" "$OUT_DIR/absolute"
APESDK_FULL_DATE="$FULL_DATE" "$ROOT/scripts/run_profile_compiler_matrix_gate.sh" "$OUT_DIR/profile-compiler"
