...
if [ "${APESDK_RUN_EXACT_CORPUS_RAW_DIFFS:-0}" = "1" ]; then
    APESDK_RUN_CORPUS_RAW_DIFFS=1 "$ROOT/scripts/run_corpus_promotion_inventory.sh" "$OUT_DIR/corpus" >/dev/null
else
    "$ROOT/scripts/run_corpus_promotion_inventory.sh" "$OUT_DIR/corpus" >/dev/null
fi
