...
- `timing_stop_forever.commands` covers PTY stop behavior after the safe empty
  `run forever` path and is owned by `both/scripts/run_interactive_timing_gate.sh`.
...
```sh
./both/golden/cli/run_cli_session.expect ./simape both/golden/cli/sessions/help.commands both/golden/cli/transcripts/help.txt
./both/golden/cli/run_cli_session.expect ./simape both/golden/cli/sessions/help_errors.commands both/golden/cli/transcripts/help_errors.txt
./both/golden/cli/run_cli_session.expect ./simape both/golden/cli/sessions/state_save_load.commands both/golden/cli/transcripts/state_save_load.txt
./both/golden/cli/run_cli_session.expect ./simape both/golden/cli/sessions/run_forever_empty.commands both/golden/cli/transcripts/run_forever_empty.txt
```
...
```sh
both/scripts/build_native_simape.sh /private/tmp/apesdk_native_harness
NATIVE_SIMAPE=/private/tmp/apesdk_native_harness/simape \
RUST_SIMAPE=rust/target/debug/simape \
both/scripts/run_cli_transcripts.sh /private/tmp/apesdk_transcripts
```

Use `both/scripts/normalize_transcript.sh` only for older fixture review where
...
```sh
both/scripts/run_release_debug_gate.sh /private/tmp/apesdk_release_debug_gate
both/scripts/performance_smoke.sh /private/tmp/apesdk_performance_smoke
```

For the stricter native C gate introduced after cycle 265:

```sh
both/scripts/run_absolute_parity_ci.sh /private/tmp/apesdk_absolute_parity
```

That path does not use `both/scripts/normalize_transcript.sh`; it only removes CRLF
transport characters before diffing. It now also runs
`both/scripts/run_interactive_timing_gate.sh` and `both/scripts/run_fuzz_parity_ci.sh` so
...
pinned date. It also runs `both/scripts/run_engine_trace_value_gate.sh` for the
currently promoted direct engine startup, first-cycle baseline, invariant, and
runtime-core value subset, plus
`both/scripts/run_terrain_food_first_cycle_gate.sh` for exact startup and
first-cycle terrain/weather/food rows. It also runs
`both/scripts/run_terrain_food_value_inventory.sh` for the still-open multi-day
terrain/weather/food inventory and
`both/scripts/run_selected_being_value_inventory.sh` for exact selected-being
first-cycle native initialization plus after-day runtime drift inventory.

Use `both/scripts/run_pending_corpus_inventory.sh` to list the current blockers for
...
```sh
both/scripts/run_platform_absolute_parity_gate.sh /private/tmp/apesdk_platform_gate
both/scripts/run_profile_compiler_matrix_gate.sh /private/tmp/apesdk_profile_matrix
both/scripts/run_required_absolute_parity_pipeline.sh /private/tmp/apesdk_required_absolute
```
