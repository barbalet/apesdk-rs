...
- Promote the native command-line save/open roundtrip into the broader strict
  ...
  before-save/after-open state continuity is exact. Selected save/open
  sidecars are exact immediately after open and after one post-load minute.
  One post-load day remains inventory-only, with open sidecar buckets for
  selected random state, movement/energy/body, brain/social/episodic, and
  immune/inventory continuity. Remaining save/open drift is in C/Rust runtime
  transcript speed, post-load day advancement, and artifact byte continuity
  promotion.
...
- Promote direct C state trace emitters for deeper terrain, braincode, social,
  ...
  selected-being drift is now bucketed with expanded sidecars for random seed,
  goals, social coordinates, brain state, attention, shout bytes, terrain/food,
  body posture/parasites/crowding/inventory, and full first immune sample.
  The day-one slice inventory narrows the earliest selected divergences to body
  posture at minute 60, honor/drives at minute 120, and movement/energy with
  terrain/food context at minute 180.
- Promote broader raw native binary artifact bytes. The direct `tranfer_out()`
  ...
  difference for all five populated scenarios at the `brpro=` token after the
  raw `awako=` writer gap was closed.
...
- Promote brain/social/episodic/immune runtime buckets from inventory to exact.
  The runtime inventory now marks brain/probe, social, and episodic sidecars as
  first diverging at minute 60, with immune and selected random state first
  diverging at minute 180. The focused immune/episodic runtime inventory keeps
  immune drift, episodic drift, and energy/random coupling split into separate
  mutation-checked buckets.
...
- Final signoff readiness: `scripts/run_final_signoff_readiness.sh` currently
  ...
  promotion. The cycle 711-740 run completed with `status=blocked blockers=5`.
  Strict mode remains intentionally non-zero until those categories are exact.
