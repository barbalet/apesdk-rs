...
- `scripts/run_selected_minute_trace_inventory.sh` extracts the direct C/Rust
  ...
  for the final 16 minutes. The rows now include selected identity, random
  seed, goals, social coordinates, brain state, attention, probe zero, script
  override, shout bytes, expanded social/episodic/territory samples, body
  sidecars, terrain/food sidecars, first immune antigen/antibody/shape/seed
  values, inventory, and preference. The gate requires schema and row-count
  parity and records the current value diff as inventory until movement, drive,
  brain, social, episodic, immune, and random-state runtime values are exact.
...
- `scripts/run_immune_episodic_runtime_inventory.sh` extracts the same
  selected-minute rows into focused immune, episodic, and energy/random
  coupling buckets. It records the current immune divergence at minute 180,
  episodic divergence at minute 60 and minute 660, and proves the immune bucket
  fails on a deliberate one-field mutation.
