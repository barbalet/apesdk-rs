...
- Save/open promotion inventory:
  ...
  advancement. It also emits selected save/open sidecars for immediate,
  post-load minute, and post-load day snapshots. One post-load minute is exact;
  one post-load day remains inventory-only with selected random state,
  movement/energy/body, brain/social/episodic, and immune/inventory buckets
  still open. The optional raw C/Rust transcript diff remains blocked until
...
- Runtime bucket inventories:
  ...
  brain/social/episodic/immune fields, while
  `scripts/run_immune_episodic_runtime_inventory.sh` splits immune,
  episodic, and energy/random coupling drift into focused mutation-checked
  buckets. These inventories keep the long seeded and multi-day corpus
  blockers tied to concrete native trace rows. The refreshed selected-minute
  schema now includes random seed, goals, social coordinates, brain state,
  attention, shout bytes, expanded social/episodic/territory samples,
  terrain/food, body sidecars, and full first immune antigen/antibody sample
  values.
...
- Populated raw byte-diff inventory:
  ...
  value gate output, records first byte-diff offsets plus nearest transfer
  sections and field tokens for all five populated artifacts, and includes a
  mutation check. After closing the raw `awako=` writer gap, all populated raw
  byte diffs currently start at `brpro=` tokens inside `being{` sections.
