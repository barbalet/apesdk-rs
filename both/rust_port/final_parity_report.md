...
Cycle 681-710 update: selected-minute runtime traces now include expanded
brain/social/body/terrain/food/immune/random sidecars. The port is still not at
1:1 runtime parity, but the first mismatches are sharper: brain/probe, social,
episodic, and body posture at minute 60; honor/drives at minute 120; and
movement, energy, immune, and selected random state at minute 180.

Cycle 711-740 update: immune/episodic runtime, save/open sidecars, and
populated raw byte diffs are now stricter and more local. Save/open is exact
through one post-load minute, but the one-day sidecar comparison still has four
open buckets. The populated raw writer now matches C's `awako=0` field, moving
the first raw byte mismatch to `brpro=` across the five populated artifacts.
Final readiness still reports `status=blocked blockers=5`, so this is not yet
a 1:1 native C experience.
