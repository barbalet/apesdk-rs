...
Cycle 681-710 update: selected-minute C/Rust traces now carry expanded runtime
sidecars for random seed, goals, social coordinates, brain state, attention,
probe zero, shout bytes, social/episodic/territory samples, body values,
terrain/food, inventory, and full first immune antigen/antibody/shape/seed
values. The refreshed inventories keep final readiness blocked, but with
narrower first mismatches: brain/probe, social, and episodic sidecars at minute
60; body posture at minute 60; honor/drives at minute 120; and movement,
energy, immune, and selected random-state drift at minute 180.

Cycle 711-740 update: the absolute pipeline now includes a focused
immune/episodic runtime inventory, selected save/open sidecar continuity, and
nearest-token populated raw byte-diff classification. Save/open sidecars are
exact immediately after open and after one post-load minute, but one post-load
day remains inventory-only with four open sidecar buckets. Populated raw byte
promotion still has five pending scenarios; after closing the raw `awako=`
writer gap, the first remaining mismatch in each scenario is at `brpro=`.
Final readiness completes and reports `status=blocked blockers=5`.
