...
APESDK_FULL_DATE="$FULL_DATE" CFLAGS="-O2" "$ROOT/both/scripts/build_native_simape.sh" "$NATIVE_RELEASE_DIR" >/dev/null
APESDK_FULL_DATE="$FULL_DATE" CFLAGS="-O0" "$ROOT/both/scripts/build_native_simape.sh" "$NATIVE_DEBUG_DIR" >/dev/null
APESDK_FULL_DATE="$FULL_DATE" cargo build --manifest-path "$ROOT/rust/Cargo.toml" --release -p simape >/dev/null

run_one() {
    local profile="$1"
    local binary="$2"
    local session="$3"
    local commands="$ROOT/both/golden/cli/sessions/$session.commands"
    ...
    "$ROOT/both/scripts/transport_normalize_transcript.sh" "$output" > "$TRANSPORT_DIR/$profile/$session.txt"
}

for session in "${sessions[@]}"; do
    run_one native-release "$NATIVE_RELEASE_DIR/simape" "$session"
    run_one native-debug "$NATIVE_DEBUG_DIR/simape" "$session"
    run_one rust-debug "$ROOT/rust/target/debug/simape" "$session"
    run_one rust-release "$ROOT/rust/target/release/simape" "$session"
...

{
    ...
    printf 'rust_release=%s\n' "$ROOT/rust/target/release/simape"
