...
cp "$ROOT/both/golden/traces/final_gate_summary.trace" "$OUT_DIR/trace_expected.trace"
...
if "$ROOT/both/scripts/trace_diff.sh" "$OUT_DIR/trace_expected.trace" "$OUT_DIR/trace_mutated.trace" >/dev/null 2>&1; then
