...
run_split() {
    ...
    "$ROOT/both/scripts/canonicalize_malformed_loader_transcript.sh" "$out_dir/combined.txt" > "$CANONICAL_DIR/$profile/$case_name.txt"
}
...
for case_file in "${cases[@]}"; do
    ...
    run_split rust "$ROOT/rust/target/debug/simape" "$case_name" "$fixture"
