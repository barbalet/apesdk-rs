...
"$ROOT/both/scripts/canonicalize_closed_stdin_transcript.sh" "$canonical_closed" > "$OUT_DIR/closed_canonical.out"
"$ROOT/both/scripts/canonicalize_closed_stdin_transcript.sh" "$missing_failure" > "$OUT_DIR/closed_missing_failure.out"
diff -u "$OUT_DIR/closed_canonical.out" "$OUT_DIR/closed_missing_failure.out"

if "$ROOT/both/scripts/canonicalize_closed_stdin_transcript.sh" "$extra_failure" > "$OUT_DIR/closed_extra_failure.out" 2> "$OUT_DIR/closed_extra_failure.err"; then
    echo "extra console-failure line was accepted" >&2
    exit 1
fi

if "$ROOT/both/scripts/canonicalize_closed_stdin_transcript.sh" "$missing_banner" > "$OUT_DIR/closed_missing_banner.out" 2> "$OUT_DIR/closed_missing_banner.err"; then
...
"$ROOT/both/scripts/canonicalize_eof_verbose_command_transcript.sh" "$verbose_no_failure" > "$OUT_DIR/verbose_no_failure.out"
"$ROOT/both/scripts/canonicalize_eof_verbose_command_transcript.sh" "$verbose_one_failure" > "$OUT_DIR/verbose_one_failure.out"
diff -u "$OUT_DIR/verbose_no_failure.out" "$OUT_DIR/verbose_one_failure.out"

if "$ROOT/both/scripts/canonicalize_eof_verbose_command_transcript.sh" "$verbose_extra_failure" > "$OUT_DIR/verbose_extra_failure.out" 2> "$OUT_DIR/verbose_extra_failure.err"; then
    echo "verbose EOF extra console-failure line was accepted" >&2
    exit 1
fi

if "$ROOT/both/scripts/canonicalize_eof_verbose_command_transcript.sh" "$verbose_missing_help" > "$OUT_DIR/verbose_missing_help.out" 2> "$OUT_DIR/verbose_missing_help.err"; then
