...
static void emit_selected_minute(const char *label, n_uint minute)
{
    simulated_group *group = sim_group();
    simulated_being *being = 0L;
    n_int terrain_height = 0;
    n_int terrain_slope_x = 0;
    n_int terrain_slope_y = 0;
    n_int terrain_water = 0;
    n_int terrain_intertidal = 0;
    n_byte terrain_food_type = 0;
    n_int terrain_food_energy = 0;
    if ((group != 0L) && (group->num > 0))
    {
        being = group->select;
        if (being == 0L)
    ...
    if (being != 0L)
    {
        ...
        being_space(being, &location_vector);
        land_vect2(&slope_vector, &terrain_height, &location_vector);
        terrain_slope_x = slope_vector.x;
        terrain_slope_y = slope_vector.y;
        terrain_water = (WATER_TEST(terrain_height, land_tide_level()) != 0);
        terrain_intertidal = terrain_height <= TIDE_MAX;
        terrain_food_classification(
    ...
    printf(
        ...
        "selected=%u name=%u:%u energy=%ld loc=%ld:%ld facing=%u speed=%u:%u honor=%u "
        "height=%ld mass=%ld posture=%u parasites=%ld crowding=%u awake=%u state=%u "
        "drives=%u:%u:%u:%u seed=%u:%u goal=%u:%u:%u:%u socialcoord=%u:%u:%u:%u "
        "brain=%u:%u:%u brainstate=%u:%u:%u:%u:%u:%u attention=%u:%u:%u:%u:%u:%u "
        "probe0=%u:%u:%u:%u:%u:%u script=%u shout=%u:%u:%u:%u:%u:%u "
        "social0=%u:%u:%u:%u:%u:%u episodic0=%u:%u:%u:%u territory0=%u:%u "
        "conception=%lu family=%u:%u:%u:%u:%u:%u terrain=%ld:%ld:%ld:%ld:%ld food=%u:%ld "
        "immune=%u:%u:%u:%u:%u:%u inventory=%u:%u preference=%u\n",
        label,
        ...
        (being != 0L) ? being->delta.mass : 0,
        (being != 0L) ? being_posture(being) : 0,
        (being != 0L) ? being_parasites(being) : 0,
        (being != 0L) ? being->delta.crowding : 0,
        (being != 0L) ? being->delta.awake : 0,
        ...
        (being != 0L) ? being->changes.drives[2] : 0,
        (being != 0L) ? being->changes.drives[3] : 0,
        (being != 0L) ? being->delta.random_seed[0] : 0,
        (being != 0L) ? being->delta.random_seed[1] : 0,
        (being != 0L) ? being->delta.goal[0] : 0,
        (being != 0L) ? being->delta.goal[1] : 0,
        (being != 0L) ? being->delta.goal[2] : 0,
        (being != 0L) ? being->delta.goal[3] : 0,
        (being != 0L) ? being->delta.social_coord_x : 0,
        (being != 0L) ? being->delta.social_coord_y : 0,
        (being != 0L) ? being->delta.social_coord_nx : 0,
        (being != 0L) ? being->delta.social_coord_ny : 0,
        (being != 0L) ? being->braindata.braincode_register[0] : 0,
        (being != 0L) ? being->braindata.braincode_register[1] : 0,
        (being != 0L) ? being->braindata.braincode_register[2] : 0,
        (being != 0L) ? being->braindata.brain_state[0] : 0,
        (being != 0L) ? being->braindata.brain_state[1] : 0,
        (being != 0L) ? being->braindata.brain_state[2] : 0,
        (being != 0L) ? being->braindata.brain_state[3] : 0,
        (being != 0L) ? being->braindata.brain_state[4] : 0,
        (being != 0L) ? being->braindata.brain_state[5] : 0,
        (being != 0L) ? being->braindata.attention[0] : 0,
        (being != 0L) ? being->braindata.attention[1] : 0,
        (being != 0L) ? being->braindata.attention[2] : 0,
        (being != 0L) ? being->braindata.attention[3] : 0,
        (being != 0L) ? being->braindata.attention[4] : 0,
        (being != 0L) ? being->braindata.attention[5] : 0,
        (being != 0L) ? being->braindata.brainprobe[0].type : 0,
        (being != 0L) ? being->braindata.brainprobe[0].position : 0,
        (being != 0L) ? being->braindata.brainprobe[0].address : 0,
        (being != 0L) ? being->braindata.brainprobe[0].frequency : 0,
        (being != 0L) ? being->braindata.brainprobe[0].offset : 0,
        (being != 0L) ? being->braindata.brainprobe[0].state : 0,
        (being != 0L) ? being->braindata.script_overrides : 0,
        (being != 0L) ? being->changes.shout[0] : 0,
        (being != 0L) ? being->changes.shout[1] : 0,
        (being != 0L) ? being->changes.shout[2] : 0,
        (being != 0L) ? being->changes.shout[3] : 0,
        (being != 0L) ? being->changes.shout[4] : 0,
        (being != 0L) ? being->changes.shout[5] : 0,
        (being != 0L) ? being->events.social[0].first_name[0] : 0,
        (being != 0L) ? being->events.social[0].family_name[0] : 0,
        (being != 0L) ? being->events.social[0].familiarity : 0,
        (being != 0L) ? being->events.social[0].friend_foe : 0,
...
"$COMPILER" "${CFLAGS_ARGS[@]}" "${DEFINES_ARGS[@]}" -I"$C_ROOT" -c "$PROBE" -o "$BUILD_DIR/c_engine_trace_probe.o" -w
objects+=("$BUILD_DIR/c_engine_trace_probe.o")

"$COMPILER" "${CFLAGS_ARGS[@]}" "${DEFINES_ARGS[@]}" -I"$C_ROOT" -o "$OUT_DIR/c_engine_trace_probe" "${objects[@]}" -lz -lm -lpthread
