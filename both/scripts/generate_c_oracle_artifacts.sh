...
APESDK_FULL_DATE="$FULL_DATE" "$ROOT/both/scripts/build_native_simape.sh" "$OUT_DIR/native-build" >/dev/null
...
for commands in "$ROOT"/both/golden/cli/sessions/*.commands; do
    ...
    mkdir -p "$run_dir"
    ( cd "$run_dir" && "$ROOT/both/golden/cli/run_cli_session.expect" "$OUT_DIR/native-build/simape" "$commands" "$OUT_DIR/transcripts/$name.txt" )
done
