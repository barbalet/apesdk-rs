...
for exact_case in "${exact_cases[@]}"; do
    ...
    run_expect "$ROOT/rust/target/debug/simape" "$commands" "$RAW_DIR/rust/$name.txt"
    "$ROOT/both/scripts/canonicalize_malformed_loader_transcript.sh" "$RAW_DIR/native/$name.txt" > "$TRANSPORT_DIR/native/$name.txt"
    "$ROOT/both/scripts/canonicalize_malformed_loader_transcript.sh" "$RAW_DIR/rust/$name.txt" > "$TRANSPORT_DIR/rust/$name.txt"
    diff -u "$TRANSPORT_DIR/native/$name.txt" "$TRANSPORT_DIR/rust/$name.txt"
done

APESDK_FULL_DATE="$FULL_DATE" "$ROOT/both/scripts/run_malformed_loader_oracle.sh" "$INVENTORY_DIR/oracle" >/dev/null
