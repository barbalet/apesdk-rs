...
for scenario in "${SCENARIOS[@]}"; do
    ...
    "$ROOT/rust/target/debug/simape" --native-raw-summary "$scenario" "$artifact" > "$rust_line"
...
for scenario in "${EXACT_BYTE_SCENARIOS[@]}"; do
    ...
    "$ROOT/rust/target/debug/simape" --native-raw-roundtrip "$artifact" "$roundtrip"
...
for scenario in "${POPULATED_SCENARIOS[@]}"; do
    ...
    roundtrip="$ROUNDTRIP_DIR/$scenario.native"
    "$ROOT/rust/target/debug/simape" --native-raw-roundtrip "$artifact" "$roundtrip"
    "$ROOT/rust/target/debug/simape" --native-raw-summary "$scenario" "$roundtrip" > "$RUST_DIR/$scenario.roundtrip.trace"
