...
"$ROOT/both/scripts/run_selected_minute_value_gate.sh" "$OUT_DIR/selected-minute-values" >/dev/null
"$ROOT/both/scripts/run_selected_being_value_inventory.sh" "$OUT_DIR/selected-being-values" >/dev/null
"$ROOT/both/scripts/run_after_day_drift_inventory.sh" "$OUT_DIR/after-day-drift" >/dev/null
"$ROOT/both/scripts/run_after_day_slice_inventory.sh" "$OUT_DIR/after-day-slices" >/dev/null
"$ROOT/both/scripts/run_brain_social_runtime_inventory.sh" "$OUT_DIR/brain-social-runtime" >/dev/null
"$ROOT/both/scripts/run_immune_episodic_runtime_inventory.sh" "$OUT_DIR/immune-episodic-runtime" >/dev/null
"$ROOT/both/scripts/run_selected_minute_trace_inventory.sh" "$OUT_DIR/selected-minute-trace" >/dev/null
"$ROOT/both/scripts/run_save_open_continuity_inventory.sh" "$OUT_DIR/save-open-continuity" >/dev/null
"$ROOT/both/scripts/run_native_raw_binary_value_gate.sh" "$OUT_DIR/native-raw-binary-values" >/dev/null
APESDK_RAW_VALUE_GATE_DIR="$OUT_DIR/native-raw-binary-values" "$ROOT/both/scripts/run_populated_raw_byte_diff_inventory.sh" "$OUT_DIR/populated-raw-byte-diff" >/dev/null
"$ROOT/both/scripts/run_corpus_promotion_inventory.sh" "$OUT_DIR/corpus-promotion" >/dev/null
...
immune_episodic_manifest="$OUT_DIR/immune-episodic-runtime/immune_episodic_runtime_inventory_manifest.txt"
...
immune_episodic_runtime_status="$(awk -F= '$1 == "immune_episodic_runtime_status" { print $2 }' "$immune_episodic_manifest")"
immune_episodic_open_buckets="$(awk -F= '$1 == "open_bucket_count" { print $2 }' "$immune_episodic_manifest")"
...
post_load_day_sidecar_status="$(awk -F= '$1 == "post_load_day_sidecar_status" { print $2 }' "$save_open_manifest")"
post_load_day_sidecar_open_buckets="$(awk -F= '$1 == "post_load_day_sidecar_open_buckets" { print $2 }' "$save_open_manifest")"
...
[ "$minute_status" = "exact" ] && [ "$brain_social_runtime_status" = "exact" ] && [ "$immune_episodic_runtime_status" = "exact" ] || blockers=$((blockers + 1))
[ "$raw_diff_status" = "exact" ] && [ "$post_load_minute_status" = "exact" ] && [ "$post_load_day_status" = "exact" ] && [ "$post_load_day_sidecar_status" = "exact" ] || blockers=$((blockers + 1))
...

{
    ...
    printf 'immune_episodic_runtime_status=%s\n' "$immune_episodic_runtime_status"
    printf 'immune_episodic_open_buckets=%s\n' "$immune_episodic_open_buckets"
    printf 'immune_episodic_manifest=%s\n' "$immune_episodic_manifest"
    ...
    printf 'save_open_post_load_day_sidecar_status=%s\n' "$post_load_day_sidecar_status"
    printf 'save_open_post_load_day_sidecar_open_buckets=%s\n' "$post_load_day_sidecar_open_buckets"
