...
mkdir -p "$OUT_DIR"
APESDK_FULL_DATE="$FULL_DATE" "$ROOT/both/scripts/generate_native_raw_binary_oracle.sh" "$OUT_DIR/first" >/dev/null
APESDK_FULL_DATE="$FULL_DATE" "$ROOT/both/scripts/generate_native_raw_binary_oracle.sh" "$OUT_DIR/second" >/dev/null
