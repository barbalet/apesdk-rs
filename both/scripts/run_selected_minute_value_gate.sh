...
extract_minute60_core() {
    ...
    function first_tuple_values(value, wanted,   parts, idx, result) {
        split(value, parts, ":")
        result = parts[1]
        for (idx = 2; idx <= wanted; idx++) {
    ...
    /^SELECTED-MINUTE / {
        ...
        if (fields["minute"] == "60") {
            printf "selected-minute60-core label=%s minute=%s date=%s time=%s population=%s selected=%s energy=%s loc=%s facing=%s speed=%s honor=%s mass=%s awake=%s state=%s drives=%s episodic0=%s immune=%s preference=%s\n",
                ...
                fields["state"],
                fields["drives"],
                first_tuple_values(fields["episodic0"], 2),
                fields["immune"],
                fields["preference"]
        }
    }
...

{
    ...
    printf 'covered=minute60-selected-energy-location-facing-speed-honor-mass-awake-state-drives-episodic-event-food-immune-preference\n'
