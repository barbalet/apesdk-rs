...
RUST_BIN="${RUST_SIMAPE:-"$ROOT/rust/target/debug/simape"}"
...
run_one() {
    ...
    if command -v expect >/dev/null 2>&1; then
        "$ROOT/both/golden/cli/run_cli_session.expect" "$binary" "$commands" "$output"
    else
        "$binary" < "$commands" > "$output" || true
    fi
}

for commands in "$ROOT"/both/golden/cli/sessions/*.commands; do
    ...
    if [ -x "$NATIVE_BIN" ]; then
        if grep -Eiq '^[[:space:]]*run[[:space:]]+forever([[:space:]]|$)' "$commands"; then
            ...
            "$ROOT/both/scripts/normalize_transcript.sh" "$OUT_DIR/native/$name.txt" > "$OUT_DIR/normalized/native_$name.txt"
        fi
    fi
    run_one "$RUST_BIN" "$commands" "$OUT_DIR/rust/$name.txt"
    "$ROOT/both/scripts/normalize_transcript.sh" "$OUT_DIR/rust/$name.txt" > "$OUT_DIR/normalized/rust_$name.txt"
done
