...
Run status for cycles 681-695: completed as refreshed native/Rust trace
sidecars and wider first-mismatch classification, not exact promotion. The
direct C probe and Rust `SELECTED-MINUTE` formatter now emit selected identity,
random seed, goal/social coordinates, brain state, attention, probe zero,
script override, shout/listen bytes, expanded social/episodic/territory,
body, terrain/food, full first immune sample, inventory, and preference
fields. `scripts/run_brain_social_runtime_inventory.sh` now classifies
brain/probe, social, episodic, immune, and random-state buckets. The refreshed
run reports brain/probe, social, and episodic first diverging at minute 60,
immune and random state first diverging at minute 180.
...
Run status for cycles 696-707: completed as widened day-one selected slice
inventory, not exact promotion. `scripts/run_after_day_slice_inventory.sh` now
compares terrain/food, body posture/parasites/crowding/inventory, goal/social
coordinates, and selected random seed in addition to the previous movement,
energy, body, and honor/drive fields. The refreshed run reports body posture
first diverging at minute 60, honor/drives at minute 120, and movement/energy
at minute 180 with terrain `105:-3:-2:1:1 -> 123:-2:-2:0:1` and food
`0:0 -> 3:30` on the selected being.
...
Run status for cycles 708-717: completed as focused immune/episodic inventory,
not immune or episodic closure. The selected-minute `immune=` field now carries
antigen, shape antigen, antibody, shape antibody, and immune random seed values.
`scripts/run_immune_episodic_runtime_inventory.sh` captures immune runtime
drift at minute 180, episodic runtime drift at minute 60 and minute 660, and
energy/random coupling drift at minute 120, with a deliberate immune mutation
check. Final readiness now blocks on this focused runtime inventory until those
...
Run status for cycles 718-725: completed as save/open selected sidecar
classification, not one-day exact closure. `save_open_continuity_fixture()` now
emits `SAVE-OPEN-SIDECAR` snapshots for before-save, after-open, continued
minute, post-open minute, continued day, and post-open day. The save/open
inventory reports exact before-save/after-open state, exact immediate
sidecars, and exact one-minute post-load sidecars. One-day post-load sidecars
remain inventory-only with four open buckets: selected random state,
movement/energy/body, brain/social/episodic, and immune/inventory continuity.
...
Run status for cycles 726-732: completed as byte-diff token narrowing and first
raw-writer closure, not populated byte exactness. The raw byte-diff inventory
now records the nearest transfer token for the first mismatch in every
populated scenario. Rust raw output now matches C's zero-valued `awako=` field
for populated beings, moving the first remaining mismatch to `brpro=` for all
five populated raw artifacts. The five populated scenarios remain
`value-exact-byte-pending`.
...
Run status for cycles 733-737: completed as stricter corpus/readiness wiring,
not exact corpus promotion. Final readiness now includes the focused
immune/episodic runtime inventory, save/open selected sidecar continuity, and
populated raw token-level byte-diff status. The exact corpus promotion remains
blocked with all five promotion corpora not ready.
...
Run status for cycles 738-740: completed as strict readiness execution, not
final signoff. `scripts/run_final_signoff_readiness.sh /private/tmp/apesdk_final_signoff_711_740`
completes and reports `status=blocked blockers=5`. The remaining blocker
families are selected/after-day runtime inventory, brain/social/episodic/immune
runtime inventory, save/open continuity, broader populated raw byte promotion,
and exact corpus promotion.
