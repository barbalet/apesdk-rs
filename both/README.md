...
The top-level tree is split by ownership:

* `../c` contains the native C ApeSDK command-line source and C build scripts.
* `../rust` contains the Rust Cargo workspace for the `simape` command-line port.
* `../both` contains shared documentation, golden fixtures, parity scripts, and C/Rust comparison notes.
